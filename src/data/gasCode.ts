...
function doGet(e) {
  // บันทึก Spreadsheet ID หากมีส่งมา
  if (e && e.parameter && e.parameter.spreadsheetId) {
    try {
      PropertiesService.getScriptProperties().setProperty('LAST_SPREADSHEET_ID', e.parameter.spreadsheetId);
    } catch(err) {}
  }
...
function getSpreadsheet() {
  try {
    var savedId = PropertiesService.getScriptProperties().getProperty('LAST_SPREADSHEET_ID');
    if (savedId && savedId !== "") {
...
function doPost(e) {
  ...
  try {
    ...
    if (spreadsheetId && spreadsheetId !== "") {
      try {
        PropertiesService.getScriptProperties().setProperty('LAST_SPREADSHEET_ID', spreadsheetId);
      } catch(err) {}
    }
